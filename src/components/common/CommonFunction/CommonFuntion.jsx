    ...
    for (let subIndex = 0; subIndex < section.length; subIndex++) {
      if (Array.isArray(section[subIndex])) {
        if (section[subIndex] && section[subIndex].length > 0) {
          for (
            let childIndex = 0;
            childIndex < section[subIndex].length;
            childIndex++
          ) {
            if (
              section[subIndex][childIndex] &&
              section[subIndex][childIndex].type === "removeIcon"
            ) {
              indexId =
                section[subIndex][childIndex] &&
                section[subIndex][childIndex].castRelationId;
            }
          }
...
export const removeAllDynamicAddfields = (params) => {
  for (let index = 0; index < params.length; index++) {
    if (params[index].sectionKey === "relationShip") {
      let section = params[index].section,
        removed = false;
      if (section && section.length > 0) {
        for (let subIndex = 0; subIndex < section.length; subIndex++) {
          removed = false;
    ...
    if (Array.isArray(section[subIndex])) {
      for (
        ...
        if (
          ...
          id =
            section[subIndex][childIndex] && section[subIndex][childIndex].id;
        }
      }
    }
  }
    ...
    sort.map((sortItem) => {
      if (sortItem?.display) {
        ...
        if (sortItem?.value && sortItem?.sortKey) {
          uri += sortItem?.sortValue
            ? `${sortItem.sortKey}=${sortItem.sortValue}&`
            : "";
        }
      }
    });
    ...
    filters
      .map((filterItem) => {
        if (
          ...
          filterItem.value instanceof Array
        ) {
          let allValues = [];
        ...
        if (
          ...
          (typeof filterItem.value === "string" ||
            filterItem.value instanceof String)
        ) {
...
export const manageDropDownValidation = (stepName, name, value) => {
    ...
    stepName?.map((obj) =>
      obj.name == updatedKey
        ? (obj["data"] = obj["data"]?.filter(
            (dataVal) =>
              !value.some((el) => el.DisplayName === dataVal.DisplayName) &&
              dataVal
          ))
        : obj
    );
  ...
  if (name == "isMultiAudio") {
    stepName?.map((obj) =>
      obj.label == "Audio Language"
        ? value
          ? ((obj.multiple = true), (obj.value = obj.value ? [obj.value] : []))
          : ((obj.multiple = false), (obj.value = null))
        : obj
    );

    stepName?.map((obj) =>
      obj.label == "Primary Language" || obj.label == "Dubbed Language"
        ? value
          ? (obj.required = true)
          : (obj.required = false)
        : obj
...
export const validateReleaseAndTelecastDate = (rootArr, name, inputVal) => {
  let fromIndex = rootArr?.findIndex(
    (data, index) => data?.name == "telecastDate"
  );
  let toIndex = rootArr?.findIndex(
    (data, index) => data?.name == "dateZee5Published"
  );

  let fromValue = name == "telecastDate" ? inputVal : rootArr[fromIndex]?.value;
  let toValue =
    name == "dateZee5Published" ? inputVal : rootArr[toIndex]?.value;
  let fromError =
    fromValue == "" ||
    toValue == "" ||
    name == "dateZee5Published" ||
    fromIndex < 0 ||
    toIndex < 0 ||
...
export const completeImagePath = (externalId, key, url, resolution = null) => {
  ...
  return resolution
    ? `${Config.imageBaseUrl}${externalId}/${key}/${resolution}/${url}`
...
export const addTag = async (data, contentType = "") => {
  for (let index in data) {
    if (!data[index]["id"]) {
      data.splice(
        index,
        1,
        ...data?.[index]?.["title"]?.split(",")?.map((obj) => ({ title: obj }))
      );
    }
  }
  ...
  for (let index in data) {
    data.splice(
      index,
      1,
      ...(await addTagToServer([data[index]], contentType))
    );
  }
  return data;
};
